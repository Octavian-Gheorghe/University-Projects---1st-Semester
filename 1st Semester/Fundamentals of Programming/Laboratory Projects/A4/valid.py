...
def isValidAdd(words, category):
    ...
    Output: r - True if the command is valid and False otherwise
    '''
...
def isValidInsert(words, category):
    '''
    Function that verifies if the insert command is valid
    Input: words, category
    Output: r - True if the command is valid and False otherwise
    '''
...
def isValidRemove(words, category):
    ...
    Input: words, category
    Precondition: words - list, category - list
    Output: cnt - integer (3, if remove all expenses for a day, 4 if it removes between 2 days, 5 if it removes for a specific category, 0 otherwise)
    '''
    cnt = 0
    if isDay(words[1]):
        if len(words) == 2:
...
def isValidList(words, category):
    ...
    Input: words, category
    Precondition: words - list, category - list
    Output: cnt - integer (6 if command is to write the entire list, 7 if the command is to write the expenses for a given category, cnt = 8 if the command is to write the expenses with a given property, 0 otherwise)
    '''
    properties = ["<", ">", "="]
    cnt = 0
    if len(words) == 1:
...
def isValidSum(words, category):
    ...
    Input: words, category
    Precondition: words - list, category - list
    Output: r - boolean (True if the command is valid)
    '''
...
def isValidsSort(words, category):
    ...
    Precondition: words - list
                  category - list
    Output: cnt - integer (11 if command is to sort the total daily expenses in ascending order by amount of money, 12 if command is to sort the daily expenses for a category in ascending order by amount of money)
    '''
    cnt = 0
...
def isValidFilter(words, category):
    ...
    Input: words, category
    Precondition: words - list, category - list
    Output: cnt - integer (13 if command is to keep only the expenses from a given category, 14 if command is to keep only the expenses with a given property)
    '''
    properties = ["<", ">", "="]
    cnt = 0
...
def isValid(words):
    ...
    Output :        cnt = 1 add a new expense on the current day
